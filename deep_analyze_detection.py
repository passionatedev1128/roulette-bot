...
def analyze_preprocessing(roi: np.ndarray, template: np.ndarray, detector: ScreenDetector) -> Dict:
    ...
    if needs_resize:
        ...
        if roi_resized.std() < 5:
            print(f"   ⚠️  WARNING: Very low std after resize - image might be too uniform")
    else:
...
def analyze_template_quality(template_path: Path) -> Dict:
    ...
    if img.std() < 10:
        print(f"   ⚠️  WARNING: Very low std ({img.std():.1f}) - template might be too uniform")
    
    # Check contrast
    contrast = img.max() - img.min()
    print(f"   Contrast: {contrast}")
    if contrast < 50:
        print(f"   ⚠️  WARNING: Low contrast - template might not be clear")
    ...
    if black_ratio > 0.8:
        print(f"   ⚠️  WARNING: Template is mostly black")
    if white_ratio > 0.8:
        print(f"   ⚠️  WARNING: Template is mostly white")
...
def main():
    ...
    if best_score > 0.5:
        ...
        print(f"⚠️  Weak match. Score: {best_score:.3f}")
