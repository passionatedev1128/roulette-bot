...
def analyze_template(template_path, test_image_path):
    ...
    if fill_ratio < 0.1:
        print("  ⚠️  WARNING: Template has very few non-zero pixels")
    ...
    if variance < 100:
        print("  ⚠️  WARNING: Low variance - template might be too uniform/generic")
    ...
    if max_val > 0.75:
        print("  ⚠️  WARNING: High match on image with no numbers!")
...
def compare_templates(templates_dir='templates'):
    ...
    if problematic:
        print("⚠️  POTENTIALLY PROBLEMATIC TEMPLATES:")
        print()
